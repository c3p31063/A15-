...
import requests
from django.utils import timezone
from .models import CheckJob  # NoSQLへ移行する場合は後述
        ...
        if form.is_valid():
            ...
            try:
                if kind == "image":
                    ...
                    data = {"prompt": prompt}
                    r = requests.post(f"{FASTAPI_URL}/check/image", files=files, data=data, timeout=120)
                    r.raise_for_status()
                    res = r.json()

                    # --- ここで保存（RDB 版※後でNoSQLに差し替え可能） ---
                    job.result = CheckResult.objects.create(
                        job=job,
                        ...
                        raw_json=res,  # JSONField
                    )
                    for i, e in enumerate(res.get("evidences", []), start=1):
                        Evidence.objects.create(job=job, rank=i, url=e.get("url",""))
                    for i, s in enumerate(res.get("similar_images", []), start=1):
                        SimilarImage.objects.create(
                            job=job, rank=i,
                            match_url=s.get("url",""),
                            match_score=float(s.get("score") or 0.0),
                ...
                job.finished_at = timezone.now()
                job.save()
                messages.error(request, f"チェックに失敗しました: {e}")
    else:
        form = UploadForm()
